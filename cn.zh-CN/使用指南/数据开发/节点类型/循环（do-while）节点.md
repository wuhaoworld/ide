...
循环（do- while）节点支持ODPS SQL、SHELL和Python三种赋值语言。选择ODPS SQL赋值语言时，您可以通过`case when`语句进行判断。

![赋值语言](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028341110_zh-CN.png)
...
1.  进入数据开发页面，选择**新建** \> **控制** \> **do-while**。
    ...
    ![do-while](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028339350_zh-CN.png)

2.  填写**新建节点**对话框中的配置，单击**提交**。

    ![循环5次](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028339469_zh-CN.png)

3.  双击新建的do-while节点，对循环体进行定义。

    ![定义循环体](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028339471_zh-CN.png)
    ...
    -   SQL节点是Dataworks提供的一个业务处理节点示例，此处需要将其删除，替换为自己的业务处理Shell节点（打印当前循环次数）。
        ...
        ![打印循环次数](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028339477_zh-CN.png)

    -   end节点有标记循环结束和判断是否开启下一次循环两大功能，此处对do-while节点的结束条件进行定义。

        end节点本质上是一个赋值节点，仅输出true/false两种字符串，分别代表继续下一个循环和不再继续循环。

        ![赋值语言](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028339479_zh-CN.png)

        在打印循环次数和end节点中，都用到了$\{dag.loopTimes\}变量。它是系统的保留变量，代表当前的循环次数，从1开始，do-while的内部节点可以直接引用该变量。

        代码中把`dag.loopTimes`和5进行比较，可以限制整体的循环次数。第一次循环dag.loopTimes为1、第二次为2，以此类推，第五次为5。至此表达式$\{dag.loopTimes\}<5结果为false，退出循环。

4.  执行do-while节点。

    您可以根据自身需求进行调度配置后，将do-while节点提交至运维中心执行。

    -   外层do-while节点：do-while节点整体在运维中心中被当作一个整体节点来展示。如果您想要查看do-while节点的循环详情，可以右键单击节点，选择**查看内部节点**，即可跳转至内部节点视图。

        ![查看内部节点](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028439504_zh-CN.png)

    -   内部循环体：该视图分三个部分。

        ![内部循环体](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028439505_zh-CN.png)
...
5.  查看运行结果。
    ...
    ![查看运行结果](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028439507_zh-CN.png)

    您也可以查看第3次循环和第5次循环中，end节点的执行日志。

    ![执行日志](images/39508_zh-CN.png "第3次循环end节点的执行日志")

    ![执行日志](images/39509_zh-CN.png "第5次循环end节点的执行日志")
...
3.  双击新建的do-while节点，对循环体进行定义。
    1.  本示例需要给do-while节点添加一个上游节点（数据集初始化），它会产生测试数据集。
        ...
        ![测试数据集](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028439529_zh-CN.png)

    2.  创建节点后，对do-while节点进行调度配置，为其单独配置一个**节点上下文**，key为input，值为上游节点数据集初始化的outputs。

        ![节点上下文](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028539531_zh-CN.png)

        ![调度属性](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028539533_zh-CN.png)

    3.  输入业务处理节点（打印每一行数据）的代码。

        ![打印每行数据](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028539534_zh-CN.png)

        -   `dag.offset`：Dataworks系统的保留变量，代表每一次循环次数相对于第一次的偏移量。即第一次循环中offset为0、第二次为1、第三次为2…第n次为n-1。
        -   `dag.input`：该变量是您配置的do-while上下文依赖，上文中提到do-while节点配置了一个叫input的上下文依赖，值为上游节点（数据集初始化）的outputs。

            do-while内部节点可以直接用$\{dag.$\{ctxKey\}\}的方式，引用上下文依赖的值。因为上文中配置的上下文依赖key为input，因此可以使用$\{dag.input\}来引用该值。
    ...
    4.  定义end节点的结束条件。如下图所示，把dag.loopTimes与dag.input.length变量进行比较，小于则输出True继续循环，不小于则输出False退出循环。
        ...
        ![比较变量](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028539552_zh-CN.png)
...
4.  执行节点并查看执行结果。
    -   数据集初始化节点最终会产生0和1两条数据。
        ...
        ![查看结果](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/90301/156397028639556_zh-CN.png)

    -   打印每一行数据节点执行结果如下所示。

        ![打印第1行数据](images/39560_zh-CN.png "打印第1行数据")

        ![打印第2行数据](images/39562_zh-CN.png "打印第2行数据")

    -   end节点的结果如下所示。

        ![第1次end](images/39565_zh-CN.png "第1次end执行结果")

        ![第2次end](images/39566_zh-CN.png "第2次end执行结果")
