...
MaxCompute Writer插件用于实现向MaxCompute中插入或者更新数据，主要适用于开发者，可以将业务数据导入MaxCompute，适合于TB、GB等数量级的数据传输。

**说明：** 在开始配置MaxCompute Writer插件前，请首先配置好数据源，详情请参见[配置MaxCompute数据源](intl.zh-CN/使用指南/数据集成/数据源配置/配置MaxCompute数据源.md#)，MaxCompute的详情请参见[什么是MaxCompute](../../../../intl.zh-CN/产品简介/什么是MaxCompute.md#)。

根据您配置的源头项目/表/分区/表字段等信息，在底层实现上可通过Tunnel将数据写入MaxCompute中。常用的Tunnel命令请参见[上传下载命令](../../../../intl.zh-CN/开发/数据上传下载/上传下载命令.md#)。

对于MySQL、MaxCompute等强schema类型的存储，数据集成会将源数据逐步读取到内存中，并根据您目的端数据源的类型，将源头数据转换为目的端对应的格式，写入目的端存储。
...
``` {#codeblock_2lf_ymj_965}
{
    ...
    "version":"2.0",//版本号
    "steps":[
        {//下面是以stream为例子，如果是其他的插件，可以查找相应的插件进行填写。
            "stepType":"stream",
            "parameter":{},
            "name":"Reader",
            "category":"reader"
...
**说明：** truncate为true的情况下，会将指定分区\\表的数据全部清理，请谨慎使用。
