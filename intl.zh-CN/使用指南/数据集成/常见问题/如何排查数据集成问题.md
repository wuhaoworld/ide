...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16281/15671586418679_zh-CN.jpg)
...
-   ADS权限问题。
    ...
    如果是RDS（或其它非MaxCompute数据源）\>ADS的数据同步，实现逻辑为先将数据装载到MaxCompute临时表，再从MaxCompute临时表同步至ADS，中转MaxCompute项目为example\_project，中转项目账号为someone@example.com。

    权限方面：

    -   ADS官方账号至少需要有需要同步的表（即MaxCompute临时表）的describe和select权限，因为ADS系统需要获取MaxCompute需要同步的表的结构和数据信息，此部分部署时已经完成授权。
    -   中转MaxCompute对应的账号someone@example.com，要拥有向指定的ADS数据库发起load data的权限，您可以在ADS系统中添加授权。
    排查思路：

    出现此问题是因为没有设置load data权限。

    中转项目账号为someone@example.com，权限方面：ADS官方账号至少需要拥有需要同步的表（即MaxCompute临时表）的describe和select权限，因为ADS系统需要获取MaxCompute需要同步的表的结构和数据信息，此部分部署时已经完成授权，登录ADS管控台给ADS授予load data的权限。
...
-   数据源信息填写错误。
    ...
    -   DRDS连接数据超时。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16281/15671586428686_zh-CN.png)
...
-   脏数据
    ...
    -   脏数据（存储emoji）。
        ...
        -   实例ID形式添加数据源
            ...
            ![实例ID](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16281/156715864254661_zh-CN.png)
